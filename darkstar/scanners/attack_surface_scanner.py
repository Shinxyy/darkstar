...
class AttackSurfaceScanner(BaseScanner):
    ...
    async def run(self) -> Dict[str, Any]:
        ...
        logger.info(f"{Fore.GREEN}[+] Attack surface mapping completed. Running subsequent scans...{Style.RESET_ALL}")
    ...
    async def _run_nuclei_on_discovered(self, bbot_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if os.path.exists(bbot_results["subdomains_file"]) and os.path.getsize(bbot_results["subdomains_file"]) > 0:
            ...
            with ThreadPoolExecutor() as executor:
                nuclei_scanner = NucleiScanner(bbot_results["subdomains_file"], self.org_name)
    ...
    async def _run_rustscan_on_discovered(self, bbot_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if os.path.exists(bbot_results["ips_file"]) and os.path.getsize(bbot_results["ips_file"]) > 0:
            ...
            if discovered_ips:
                rustscan_dir = prepare_output_directory(self.org_name, "rustscan_discovered")
                ...
                if (isinstance(rustscan_results, dict) and "bruteforce_results" in rustscan_results):
    ...
    async def _detect_and_scan_wordpress(self, bbot_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if os.path.exists(bbot_results["subdomains_file"]) and os.path.getsize(bbot_results["subdomains_file"]) > 0:
            ...
            logger.info(f"{Fore.GREEN}[+] WordPress Domains: {Fore.CYAN}{wordpress_domains}{Style.RESET_ALL}")
            
            if wordpress_domains:
                logger.info(f"{Fore.CYAN}Running WordPress-specific nuclei scan...{Style.RESET_ALL}")
                
                with ThreadPoolExecutor() as executor:
                    wp_scanner = WordPressNucleiScanner(wordpress_domains, self.org_name)
                    await asyncio.get_event_loop().run_in_executor(executor, wp_scanner.run)
                
                return {"wordpress_domains": wordpress_domains, "wp_scanner": wp_scanner}
            else:
                logger.info("No WordPress sites found, skipping WordPress-specific scans")
                return {"wordpress_domains": None}
        else:
            logger.warning(f"{Fore.YELLOW}No subdomains file available for WordPress detection{Style.RESET_ALL}")
            return {}