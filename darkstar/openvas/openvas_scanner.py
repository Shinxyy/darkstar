...
def insert_vulnerability_to_database(vulnerability, org_name):
    ...
    """
    logger.info(f"Would insert vulnerability: {vulnerability.title} for org: {org_name}")
...
class OpenVASScanner:
    ...
    def __init__(self, org_name: str, base_url: str = None):
        ...
        self.base_url = base_url or os.getenv("OPENVAS_API_URL", "http://openvas-api:8008")
        self.vulnerabilities = []
        logger.info(f"{Fore.CYAN}OpenVAS Scanner initialized for org: {org_name}{Style.RESET_ALL}")
        logger.info(f"{Fore.CYAN}Using OpenVAS API URL: {self.base_url}{Style.RESET_ALL}")
    ...
    async def scan_targets(self, targets: List[str]) -> None:
        ...
        """
        logger.info(f"{Fore.CYAN}Starting OpenVAS scan for targets: {Fore.YELLOW}{targets}{Style.RESET_ALL}")
        ...
        async with OpenVASAPIClient(base_url=self.base_url) as openvas:
            ...
            for idx, (task, start_res) in enumerate(zip(task_results, starts)):
                if not isinstance(start_res, Exception):
                    logger.info(f"{Fore.CYAN}Processing task {task['id']} start response: {start_res}{Style.RESET_ALL}")
                    # The start response should now contain report_id explicitly
                    report_id = None
                    if isinstance(start_res, dict):
                        report_id = start_res.get("report_id")
                        logger.info(f"{Fore.CYAN}Extracted report_id: {report_id}{Style.RESET_ALL}")
                    else:
                        logger.warning(f"{Fore.YELLOW}Start response is not a dict, type: {type(start_res)}{Style.RESET_ALL}")
                    ...
                    logger.info(f"Task {task['id']} started with report ID: {report_id}")
                else:
                    logger.error(f"Task start failed: {start_res}")
            
            logger.info(f"{Fore.CYAN}Created task_info with {len(task_info)} tasks to monitor{Style.RESET_ALL}")

            # Start background monitoring
            monitor_task = asyncio.create_task(self.monitor_task_queue(openvas, task_info))

            # Wait for all tasks to complete
            await asyncio.gather(monitor_task, return_exceptions=True)

        logger.info(f"{Fore.GREEN}[+] OpenVAS: created {len(target_results)} targets and {len(task_results)} tasks, all completed{Style.RESET_ALL}")

    async def monitor_task_queue(self, openvas_client: OpenVASAPIClient, task_info: List[Dict[str, Any]]) -> None:
        """
        ...
        """
        logger.info(f"{Fore.CYAN}Starting background monitoring of {len(task_info)} OpenVAS tasks...{Style.RESET_ALL}")
        
        # Create output directory for reports
        reports_dir = f"openvas_reports_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
        os.makedirs(reports_dir, exist_ok=True)
        logger.info(f"{Fore.CYAN}Reports will be saved to: {reports_dir}{Style.RESET_ALL}")
        ...
        while True:
            ...
            if completed_count == total_count:
                logger.info(f"{Fore.GREEN}[+] All {total_count} OpenVAS tasks completed!{Style.RESET_ALL}")
                break
            
            logger.info(f"{Fore.CYAN}Progress: {completed_count}/{total_count} tasks completed{Style.RESET_ALL}")
            ...
            for task in task_info:
                ...
                try:
                    ...
                    logger.info(f"{Fore.CYAN}Task {task['task_id']} ({task['task_name']}): {current_status}{Style.RESET_ALL}")
                    
                    # Check if task is completed
                    if current_status in ["Done", "Stopped"]:
                        if task["report_id"]:
                            try:
                                logger.info(f"{Fore.CYAN}Fetching report for completed task {task['task_id']} with report_id: {task['report_id']}...{Style.RESET_ALL}")
                                logger.info(f"{Fore.CYAN}Making API call to: {openvas_client._client.base_url}/reports/{task['report_id']}{Style.RESET_ALL}")
                                
                                # Use the report_id that was captured when the task was started
                                report_xml = await openvas_client.get_report(task["report_id"])
                                ...
                                if report_xml and len(report_xml.strip()) > 0:
                                    ...
                                    logger.info(f"{Fore.GREEN}[+] Report saved: {report_filename} ({len(report_xml)} bytes){Style.RESET_ALL}")
                                    task["completed"] = True
                                else:
                                    logger.warning(f"{Fore.YELLOW}[!] Report retrieved but empty for task {task['task_id']}{Style.RESET_ALL}")
                                    task["completed"] = True
                                
                            except Exception as e:
                                logger.error(f"{Fore.RED}[-] Failed to fetch report for task {task['task_id']} with report_id {task['report_id']}: {e}{Style.RESET_ALL}")
                                task["completed"] = True  # Mark as completed to avoid infinite retry
                        else:
                            logger.warning(f"{Fore.YELLOW}[!] Task {task['task_id']} completed but no report ID available{Style.RESET_ALL}")
                            task["completed"] = True
                            
                    elif current_status in ["Failed", "Interrupted"]:
                        logger.error(f"{Fore.RED}[-] Task {task['task_id']} failed with status: {current_status}{Style.RESET_ALL}")
                        task["completed"] = True  # Mark as completed to stop monitoring
                        
                except Exception as e:
                    logger.error(f"{Fore.RED}[-] Error checking status for task {task['task_id']}: {e}{Style.RESET_ALL}")
    ...
    async def parse_results_to_vulns(self, report_file: str) -> None:
        ...
        """
        logger.info(f"{Fore.BLUE}[*] Processing findings from report: {report_file}{Style.RESET_ALL}")
        try:
            tree = ET.parse(report_file)
            root = tree.getroot()
        except ET.ParseError as e:
            logger.error(f"{Fore.RED}[!] Error parsing report XML: {str(e)}{Style.RESET_ALL}")
            return
        ...
        logger.info(f"{Fore.GREEN}[+] Processed {vulnerability_count} vulnerabilities ({skipped_count} skipped as false positives){Style.RESET_ALL}")
